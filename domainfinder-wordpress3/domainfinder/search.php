	...
	function search_host($domain, $host) {
		...
		if ($this->exception == '') {       
			...
			while (!feof($conn)) {
				...
				if (preg_match('/^Whois Server\s*:?\s*([a-z0-9-]+(\.[a-z0-9-]+)+)/i', $line, $m)) {
					$this->info_whois_server = $m[1];
				}
...
$search = new CpWidget_DomainFinder_Search();
